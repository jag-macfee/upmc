...
def solve_flat_plate_upm_init(v_o, c, n_panels):
    ...
    # A matrix
    A_mat = A_flat_plate_no_kutta_init(zeta, x)
...
def solve_flat_plate_unsteady(c, n_panels, v_gust, k_max, U_inf):
    # Initial vortex positions zeta_n as denoted in Lysak
    zeta = np.array([(c * (n - 1) / n_panels) for n in range(1, n_panels + 2)])
    ...
    # zeta_wake[k - 1] stores wake vortex x-positions corresponding to gamma_wake[k - 1]
    ...
    zeta_wake = np.zeros((1, k_max - 1))

    # March through timesteps k = 2 all the way to k = k_max
    zeta_last = zeta[-1]  # end of airfoil
    for k in range(2, k_max + 1):
        gamma, gamma_wake, zeta_wake = _advance_flat_plate_unsteady_timestep(
            k,
            n_panels,
            x,
            v,
...
def _advance_flat_plate_unsteady_timestep(
    ...
    for n in range(1, n_panels + 1):
        ...
        for i in range(1, k):
            zeta_i_wake = wake_vortex_position(zeta_last, panel_width, i)
            wake_vortex_contribution += gamma[k - i - 1, -1] / (
                2 * np.pi * (zeta_i_wake - x[n - 1])
            )
    ...
    prev_zeta_row = zeta_wake[-1]
    new_zeta_row = np.copy(prev_zeta_row)
    ...
    new_zeta_row[k - 2] = wake_vortex_position(zeta_last, panel_width, k - 1)
    zeta_wake = np.vstack((zeta_wake, new_zeta_row))

    return gamma, gamma_wake, zeta_wake
